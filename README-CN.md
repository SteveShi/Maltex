...
- **全协议支持**: 轻松处理 HTTP、FTP、BitTorrent、磁力链接 (Magnet) 等协议；启用实验内核后还支持 ED2K 与迅雷链接。
- **卓越性能**: 内置深度优化的 `aria2` 核心，在保证极速下载的同时，维持极低的内存占用。
- **实验性 aria2-next 引擎**: 可选切换到 [aria2-next](https://github.com/AnInsomniacy/aria2-next) 引擎，获得内置 `aria2c` 不具备的能力：
  - **ED2K 链接**（`ed2k://`）
  - **迅雷链接**（`thunder://`）原生解码
  - 限速支持小数单位（如 `1.5M`）
  - 仅做种分享控制（`--detach-share-only`），适用于 BT 与 ED2K
  - 可直接从任务复制磁力 / ED2K 链接
  - 引擎启动失败时自动降级到稳定版 aria2

  > 在 **偏好设置 → Aria2 → 内核来源** 中启用。由于 aria2-next 2.4.x 已回退到标准 aria2 基线，其余选项的行为与内置引擎一致。
...
- **核心引擎**: [aria2](https://aria2.github.io/) / [aria2-next](https://github.com/AnInsomniacy/aria2-next)（实验性）
...
如果遇到引擎连接失败的问题：
1. **重置引擎**: 强制结束残留的 `aria2c` 或 `aria2-next` 进程：
   ```bash
...
4. **aria2-next 问题**: 如果实验性引擎失败，应用会自动降级到稳定版 aria2 引擎。查看 `maltex.log` 了解降级信息。
